...
> 版本 v0.2（MVP 范围；2026-08-28 口径对齐）。定位：读书选择器 + 阅读痕迹作品化工具 的读前模块。
> 相比 v0.1：统一单书阅读时长字段优先级，并明确 MVP 只落决策档案，复盘卡列为二期。
...
| 单书累计阅读时长（秒） | `/book/getprogress` → `readingTime` 优先；缺失时兼容回退 `recordReadingTime` | 个性化阅读速度基线 |
...
- **个人速度基线**：`/readdata/detail(overall)` 总时长 + 最近读完 2-3 本的单书时长（`readingTime` 优先，缺失时兼容回退 `recordReadingTime`）÷ `wordCount` → 字/分钟；新用户回退到群体均值（中文 350-500 字/分钟）并标注"估算"
...
Step 6 归档与闭环（本产品的飞轮，也是二期"读后作品"的接口）
  决策档案 = 卡片快照 + 用户动作 + 时间
  本期只落决策档案；"预期 vs 现实"复盘卡为二期
  （届时对照 verdict/顾虑与实际划线、想法）→ 用于校准后续卡片 + 喂给阅读自画像
```
...
- 预计时长回测：对用户已读完的书，|预测 − 实际累计时长| / 实际累计时长 ≤ 40%；实际累计时长优先取 `readingTime`，缺失时兼容回退 `recordReadingTime`
- 决策档案落库；本期只落决策档案，复盘卡为二期
