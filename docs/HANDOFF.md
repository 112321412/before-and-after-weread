...
- 关键 Git 快照：`125a968`（Key 门安全）→ `99e2575`（书评缓存）→ `4551d14`（非空 Key 校验）→ `6e59649`（认证错误诊断）→ `c56ebd3`（访问口令）→ `613a496`（同步隔离）→ `ab58182`（决策历史）→ `c7f8813`（F2.1 单书回顾）→ `9fbad6c`（v1.3 视觉实现基线）；本轮修正提交包含错误态双动作与本文档收口。
- `npm run check:*` 全部通过，`npm run build` 零错误；视觉检查还覆盖 F2.2/F2.3 回归。dev 命令带端口自清理（`scripts/free-port.js`）。
...
4. 上述真实浏览器终验尚未完成；mock、离线 fixture 和既有错误态验证不能替代它。
